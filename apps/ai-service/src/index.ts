...
import { generateIntakeSummary, generateVoiceNote } from "./providers";
    ...
    if (route === "POST /ai/intake/summary") {
      ...
      const row = await prisma.aIDraft.create({
        data: {
          ...
          modelName: "stub-intake-v1",
          modelVersion: "0.0.1",
          draft,
          status: "PENDING",
        },
      });
      log.info({ id: row.id, type: row.type }, "intake draft");
    ...
    if (route === "POST /ai/voice/note") {
      ...
      const row = await prisma.aIDraft.create({
        data: {
          ...
          refId: body.ref_id ?? null,
          inputHash: hashInput(body),
          modelName: "stub-voice-v1",
          modelVersion: "0.0.1",
          draft,
          status: "PENDING",
        },
      });
      log.info({ id: row.id, type: row.type }, "voice draft");
      return json(res, 201, { data: row });
    }
