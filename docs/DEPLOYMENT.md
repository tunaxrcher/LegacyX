...
| Browser DevTools shows `CORS error` calling `https://api-legacyx.unityx.group/...` | `ALLOWED_ORIGINS` not set (or doesn't include the calling app's full URL). api-server's middleware rejects the preflight. | `ALLOWED_ORIGINS=https://app-legacyx.unityx.group,https://m-legacyx.unityx.group` — note the `https://` (different format from `SERVER_ACTIONS_ALLOWED_ORIGINS`) |
| Client-side fetch still calls `http://localhost:3001/...` after env change | `NEXT_PUBLIC_*` are inlined at BUILD time, not read at runtime | Rebuild the backoffice / patient-app images (`docker compose build --no-cache backoffice-web patient-app`) — recycle alone is not enough |
