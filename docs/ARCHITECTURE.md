...
Two-Level Catalog: `ServiceCategory` (Dental / Beauty & Spa / Wellness) → `Service` (procedures ที่ patient จองได้) ผูกกับ staff-side `procedureCode` ผ่าน FK. ทั้ง category code และ service code **auto-generated จาก name** (slugified) — UI ซ่อน manual input. รูปภาพ Service upload ตรงไป S3 ผ่าน `/api/v1/uploads/service-image`. Manager จัดการที่ `/manager/services`
...
- `/audit` — Audit Log viewer (filter by actor / event / branch + correlation_id deep-link, MANAGER)
- `/break-glass` — Break-Glass override approval queue (MANAGER)
- `/dlq` — Dead Letter Queue viewer + manual reprocess button (ADMIN)
