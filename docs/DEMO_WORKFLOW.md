...
| 3.3 | `/visits/<id>` → SOAP tab → AI Assistant | (option) คลิก **Load draft** เพื่อดึง AI draft ของ EMR ที่ pending · ตรวจ + แก้ไข | ✅ ใช้เนื้อหา draft เป็นพื้นฐานของ SOAP note |
| 3.4 | `/visits/<id>` → SOAP tab → ปุ่ม **Sign** | กดเซ็น EMR ของ visit นี้ | ✅ Emit `emr.signed` (ABAC scope=`self` — แพทย์เซ็นได้เฉพาะของตัวเอง) |
...
2. Worker handler `appointment-created.notify` → insert `NotificationLog` row (templateCode `appointment.confirmed`, status `PENDING`)
...
5. ที่ Backoffice → `0800000002` (MANAGER) → `/manager/notifications` → เห็นการ์ด KPI (PENDING/SENT/FAILED) + row นี้กับสถานะ `SENT`
...
4. Manager เปิด `/manager/notifications` → filter channel=EMAIL → เห็นรายการเตือนทั้งหมด
...
3. `/manager/resources` — เพิ่ม/แก้ไข/Retire ห้องเครื่องมือ (CRUD พร้อม edit/delete · ไม่มี Release · MANAGER role)
...
|---|---|---|
| **เช็คอินแล้วไม่มีห้องให้เลือก** | ห้องที่ seed อยู่ครบไหม / สถานะ MAINTENANCE | `/manager/resources` ตรวจสถานะ + activate |
